...
<div class="container-fluid text-center">
    ...
    if ($answered_test_row):?>
        <?php if ($answered_test_row->submitted && !$marked): ?>
            ...
            <a onclick="unsubmit_test(event)" href="<?=ROOT?>/take_test/<?=$row->test_id?>/<?=$answered_test_row->user_id?>/?unsubmit=true">
                <button class="btn btn-danger float-end">unSubmit Test</button>
            </a>
            <a onclick="set_test_as_marked(event)" href="<?=ROOT?>/mark_test/<?=$row->test_id?>/<?=$answered_test_row->user_id?>/?set_marked=true">
                <button class="btn btn-secondary float-end">Set test as marked </button>
            </a>
            <a onclick="auto_mark(event)" href="<?=ROOT?>/mark_test/<?=$row->test_id?>/<?=$answered_test_row->user_id?>/?auto_mark=true">
                <button class="btn btn-warning float-end">Auto mark</button>
            </a>
        <?php else:?>
